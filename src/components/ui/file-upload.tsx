...
import { Button, Modal, Paper, Text, Group, Stack, ActionIcon, Image, Loader } from "@mantine/core";
import { IconUpload, IconFile, IconX, IconFileText, IconPhoto, IconZoomIn } from "@tabler/icons-react";
...
export function FileUpload({
  ...
  const handleFiles = async (newFiles: FileList | File[]) => {
    ...
    const filesWithPreviews = await Promise.all(validFiles.map(createFilePreview));
    ...
    <>
      ...
      <Stack gap="md">
        {files.length === 0 ? (
          ...
          >
            <IconUpload size={64} color="#999999" style={{ marginBottom: 16 }} />
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {fileItem.preview && isTextFile(fileItem.file) && (
                            ...
                            >
                              ...
                              >
                                {typeof fileItem.preview === 'string' && fileItem.preview.substring(0, 200)}
                                {typeof fileItem.preview === 'string' && fileItem.preview.length > 200 && "..."}
                              </Text>
                            </Paper>
                          )}
                          ...
                          {isWordDocument(fileItem.file) && (
                            fileItem.preview === "preview-not-supported" ? (
                            ...
                            ) : fileItem.preview && fileItem.preview.startsWith("Error") ? (
                              <Paper
                                p="xs"
                                style={{
                                  backgroundColor: "#1a1a1a",
                              ...
                              >
                                <div
                                  dangerouslySetInnerHTML={{ __html: fileItem.preview || "" }}
                                  style={{
                                    fontSize: "10px",
                                    lineHeight: 1.3,
                                    color: "#000000",
