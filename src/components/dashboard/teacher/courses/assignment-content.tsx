...
import { Assignment, AssignmentQueryParams } from "@/services/assignment-service/assignment-type"
...
export function AssignmentContent({ moduleId }: AssignmentContentProps) {
    ...
    const [assignmentTypeFilter, setAssignmentTypeFilter] = useState<string | null>(null)
    ...
    const { data, isLoading, error, refetch } = useGetAssignments(moduleId, queryParams)
    ...
    return (
        <Stack gap="md">
            ...
            {assignments.length === 0 ? (
                ...
                >
                    <Text c="dimmed">No assignments found. Create your first assignment to get started.</Text>
                </Card>
            ) : (
                <Stack gap="sm">
                    {assignments.map((assignment: Assignment) => (
                        <Card
                            ...
                            style={{
                                background: "rgba(34, 34, 34, 0.6)",
                                border: `1px solid ${getAssignmentTypeBorderColor(assignment.assignmentType)}`,
                                transition: "all 0.2s ease",
                                cursor: "pointer",
                            }}
                            onMouseEnter={(e) => {
                                e.currentTarget.style.borderColor = getAssignmentTypeBorderColor(assignment.assignmentType).replace("0.3", "0.6")
                                e.currentTarget.style.background = getAssignmentTypeColor(assignment.assignmentType)
                            }}
                            onMouseLeave={(e) => {
                                e.currentTarget.style.borderColor = getAssignmentTypeBorderColor(assignment.assignmentType)
                                ...
                                <Group gap="md" style={{ flex: 1 }}>
                                    <Box
                                        style={{
                                            ...
                                            height: 40,
                                            borderRadius: 8,
                                            background: getAssignmentTypeColor(assignment.assignmentType),
                                            border: `1px solid ${getAssignmentTypeBorderColor(assignment.assignmentType)}`,
                                            display: "flex",
                                            alignItems: "center",
                                        ...
                                        <Group gap="sm" mb={4}>
                                            ...
                                            <Badge
                                                ...
                                                style={{
                                                    ...
                                                    color: assignment.is_published ? "#bdf052" : "#f6acae",
                                                    border: `1px solid ${assignment.is_published
                                                        ? "rgba(189, 240, 82, 0.3)"
                                                        : "rgba(246, 172, 174, 0.3)"}`,
                                                }}
                                                leftSection={assignment.is_published ? <CheckCircle size={12} /> : <XCircle size={12} />}
                                            >
