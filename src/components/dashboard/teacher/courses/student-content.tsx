...
import { EnrolledStudent, StudentQueryParams } from "@/services/student-service/student-type"
...
export function StudentContent({ courseId }: StudentContentProps) {
    ...
    const { data, isLoading, error, refetch } = useGetEnrolledStudents(courseId, queryParams)
    ...
    return (
        <Stack gap="md">
            ...
            {students.length === 0 ? (
                ...
                >
                    <Text c="dimmed">No students found. Students will appear here once they enroll in your course.</Text>
                </Card>
            ) : (
                <Stack gap="sm">
                                    ...
                                    <Box style={{ flex: 1 }}>
                                        <Group gap="sm" mb={4}>
                                            ...
                                            <Badge
                                                size="sm"
                                                style={{
                                                    background: student.enrollment_status === "active"
                                                        ? "rgba(189, 240, 82, 0.2)"
                                                        : "rgba(246, 172, 174, 0.2)",
                                                    color: student.enrollment_status === "active" ? "#bdf052" : "#f6acae",
                                                    border: `1px solid ${student.enrollment_status === "active"
                                                        ? "rgba(189, 240, 82, 0.3)"
                                                        : "rgba(246, 172, 174, 0.3)"}`,
                                                }}
                                                leftSection={student.enrollment_status === "active" ? <CheckCircle size={12} /> : <XCircle size={12} />}
                                            >
                                        ...
                                        <Stack gap="xs">
                                            <Group justify="space-between">
                                                ...
                                                <Text size="xs" c="dimmed">
                                                    {student.assignments_completed}/{student.assignments_total} assignments
                                                </Text>
                                            </Group>
                                            <Progress
                                        ...
                                        <Group gap="md" mt={8}>
                                            <Group gap={4}>
                                                ...
                                                <Text size="xs" c="dimmed">
                                                    Enrolled: {format(new Date(student.enrolled_at), "MMM dd, yyyy")}
                                                </Text>
                                            </Group>
                                            <Group gap={4}>
                                                <Activity size={12} color="#9ca3af" />
                                                <Text size="xs" c="dimmed">
                                                    Last activity: {format(new Date(student.last_activity), "MMM dd, yyyy")}
                                                </Text>
                                            </Group>
                                        </Group>
                                    </Box>
