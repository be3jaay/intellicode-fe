...
import { lessonService, BulkLessonRequest } from "@/services/lesson-service/lesson-service"
...
export const useBulkLessonCreation = () => {
  ...
  return useMutation({
    mutationFn: ({ courseId, moduleId, lessons }: { 
      courseId: string
      moduleId: string
    ...
    onError: (error: any) => {
      notifications.show({
        title: "Error",
        message: error?.message || "Failed to create lessons. Please try again.",
        color: "red",
        autoClose: 5000,
      })
    }
